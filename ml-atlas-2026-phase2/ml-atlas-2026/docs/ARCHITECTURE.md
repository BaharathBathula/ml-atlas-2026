...
leaderboard
        ↓
best-model diagnostics + export
```

All preprocessing lives inside the fitted scikit-learn pipeline so transformations are learned from the training partition rather than the full dataset.
