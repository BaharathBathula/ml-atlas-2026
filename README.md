...
Phase 4 adds lightweight operational ML capabilities on top of the Dataset Lab and Optimization Lab.

### Added capabilities

- Session-scoped experiment history
- Experiment IDs and metadata capture
- Lightweight candidate/staging/champion/archived model registry
- Schema-change detection
- Population Stability Index (PSI) drift monitoring
- Missingness shift visibility
- Binary-classifier calibration curves and Brier score
- Group-level fairness diagnostics
- Learning curves for overfitting/underfitting analysis
- Batch prediction on unseen CSV/Excel files
- Probability export when available
- Downloadable model cards

### Scope boundary

The public Streamlit deployment deliberately uses session state for experiment history and registry data. This demonstrates the workflow without pretending that browser-session state is a production registry. A real deployment would back these capabilities with durable storage such as PostgreSQL, MLflow, or a managed model registry.


## Phase 5 — Production Packaging & Release

The initial ML Atlas 2026 roadmap is now complete.

### Production packaging

- FastAPI inference API
- `/health`, `/model-info`, and `/predict` endpoints
- deterministic demo-model training script
- Dockerized application
- Docker Compose for UI + API
- GitHub Actions test matrix for Python 3.11 and 3.12
- GitHub tag-based release workflow
- architecture, deployment, security, and release documentation
...
The release workflow tests the project and publishes a source bundle automatically.

## Five-phase roadmap

1. **ML Knowledge & Algorithms** — taxonomy, playground, recommender
2. **Dataset Intelligence & AutoML** — profiling, preprocessing, benchmarking
3. **Optimization & Explainability** — CV, tuning, diagnostics, explanations
4. **Production ML & Governance** — registry, drift, fairness, inference workflows
5. **Production Packaging & Release** — API, Docker, CI/CD, docs, GitHub release
